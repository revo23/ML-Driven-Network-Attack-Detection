...
**Result**  
In this NetFlow intrusion detection case study, looking at the recall score across the primary and secondary dataset, Random Forest model comes out on top. Running the confusion matrix on the primary dataset for Randomforest, we see that it had 0 false-negatives. Thus, Random Forest model was the best for this classifcation (attack vs benign) for network attack detection.
